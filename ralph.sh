...
## RALPH SCOPE GATE (read 'Autonomous loop (RALPH) scope' in IMPLEMENTATION_PLAN.md)
- Tasks are '- [ ] **N.M**' checkboxes; the phase '**DoD:**' line is the done-when.
- You may ONLY work tasks tagged [LOOP-OK]. Work the FIRST unchecked [LOOP-OK] task.
- If the next unchecked task is [GATED], or only [GATED] tasks remain: STOP. Write the
  iter-log explaining the gate, make NO code changes, do NOT commit, and exit. Never
  start, scaffold, or 'partially' do a [GATED] task — Phase 3+ is lock-free concurrency
  whose acceptance gates this box cannot validate (no ARM64; no governor; perf paranoid=4).
- NO REWARD HACKING (slopwatch): never disable/skip/[Skip]/[Ignore] a test, weaken an
  assertion, suppress a warning, add [SuppressMessage], or catch-and-swallow to get green.
  If a task can't be completed honestly, STOP and log why — do not fake completion.
- VERIFY every task with 'pwsh ./build.ps1 Build' + 'pwsh ./build.ps1 Test' (net10.0);
  paste outcomes into the iter-log. 'Log or it didn't happen.'
"; then
    EXIT_CODE=$?
    echo ""
